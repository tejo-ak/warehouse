...
def prepareFormPartner(reqData):
    requtil = Utility(reqData=reqData)
    jresp = dict({'html': render_to_string('inventory/form_partner.html', {})})
    data = {}
    if 'id' in reqData:
        id = requtil.nvlGet('id')
        if id > 0:
            cust = models.Partner.objects.get(id__exact=requtil.nvlGet('id'))
...
def getPartnerSider(reqData):
    requtil = Utility(reqData=reqData)
    pas = models.Partner.objects.filter(id__exact=requtil.nvlGet('id'))
    if pas[0] is not None:
        id = pas[0].id
        jpas = requtil.modelToDicts([pas[0]])
    html = render_to_string('inventory/form_partner_sider.html', jpas)
    return html


def pencarianPartner(reqData):
    requtil = Utility(reqData=reqData)
    farg = {};
    ...
    if requtil.nvlGet('init') is not None:
        pas = models.Partner.objects.all(
        ).order_by(
            'nama')[0:requtil.nvlGet('n', 40)]
    else: pas = models.Partner.objects.filter(**farg).order_by(
        'nama')[0:requtil.nvlGet('n', 40)]
    jdatas = [];
    for pa in pas:
...
def lookupPartner(katakunci, id, max):
    requtil = Utility()
    if max is None:
        max = 40;
    pasiens = []
    if katakunci is not None and '' != katakunci and id is None:
        print katakunci
        print max
        pasiens = models.Partner.objects.filter(
        ...
        pasiens = models.Partner.objects.all()[:max]
    jpasiens = [];
